...
export class ElectricalAbstractMux2Handler extends BaseShapeHandler {
  ...
  render(attrs: ShapeAttrs): void {
    ...
    if ('mux' == e) {
      ...
      ((h = Math.pow(2, d)), (k = (height - 16) / h));
    }
    l = 3 + 0.5 * k;
    builder.begin();
    if (1 == g) {
      (builder.moveTo(0, 0.5 * (height - 10)), builder.lineTo(10, 0.5 * (height - 10)));
    } else {
      for (m = 0; m < g; m++) {
        (builder.moveTo(0, l),
          builder.lineTo(10, l),
          builder.text(14, l + 1, 0, 0, '' + m.toString(), 'center', 'middle', 0, null, 0, 0, f),
          (l += k));
      }
    }
    if (1 == h) {
      (builder.moveTo(width - 10, 0.5 * (height - 10)), builder.lineTo(width, 0.5 * (height - 10)));
    } else {
      for (m = 0; m < h; m++) {
