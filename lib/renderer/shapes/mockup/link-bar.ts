...
export class MockupTextLinkBarHandler extends BaseShapeHandler {
  ...
  render(attrs: ShapeAttrs): void {
    ...
    for (
      ...
      w = this.measureTextSize(w, k, 'Arial,Helvetica,sans-serif', 0).width;
      t[r] = 0 === w ? 42 : w;
      u += t[r];
    }
  ...
  private renderBackground(
    ...
    for (width = 0; width < extra8; width++) {
      extra5 += extra1[width] + 2 * extra2;
    }
    extra5 = (extra5 * x) / extra3;
    extra1 = ((extra1[extra8] + 2 * extra2) * x) / extra3;
    extra1 += extra5;
    if (0 === extra8) {
      builder.rect(0, 0, extra1, y);
      builder.fill();
    } else if (extra8 === height - 1) {
      builder.rect(extra5, 0, extra1 - extra5, y);
      builder.fill();
    } else if (-1 !== extra8) {
      builder.rect(extra5, 0, extra1 - extra5, y);
      builder.fill();
  ...
  private render_buttonText(
    ...
    k = 0.5 * this.measureTextSize(p3, p5, 'Arial,Helvetica,sans-serif', 0).width;
    if (null !== p3 && '' !== p3) {
      builder.begin();
      builder.moveTo(((p1 + 0.5 * p4) * p7) / p6 - k, 0.5 * p2 + 0.5 * p5);
      builder.lineTo(((p1 + 0.5 * p4) * p7) / p6 + k, 0.5 * p2 + 0.5 * p5);
      builder.stroke();
    }
  }
