...
export class InfographicBannerSingleFoldHandler extends ActorShapeHandler {
  ...
  render(attrs: ShapeAttrs): void {
    ...
    d = Math.min(width - 2 * e, d);
    g = Math.max(0, Math.min(width - d - 2 * e, this.getStyleNumber(style, 'dx2', 0.5)));
    f = Math.min(d, f);
    builder.begin();
    builder.moveTo(g, 0);
    ...
    builder.lineTo(0, 0.5 * (height - e));
    builder.close();
    builder.fillAndStroke();
    builder.setShadow(!1);
    builder.setFillAlpha('0.05');
