...
export class IosIBgMapHandler extends BaseShapeHandler {
  ...
  private renderForeground(
    ...
    if (!builder) return;
    let f;
    x = this.getStyleValue(style, 'fillColor2', '');
    y = this.getStyleValue(style, 'strokeColor', '');
    f = this.getStyleValue(style, 'strokeColor2', '');
