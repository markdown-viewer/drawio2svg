...
export class TrapezoidHandler extends ActorShapeHandler {
  ...
  render(attrs: ShapeAttrs): void {
    ...
    // Matches TrapezoidShape.redrawPath from the platform
    ...
    const dx = fixed
      ? Math.max(0, Math.min(width * 0.5, sizeValue))
      : width * Math.max(0, Math.min(0.5, sizeValue));
